...
class DynamicHyperConnection(tf.keras.layers.Layer):
    ...
    def call(self, H, T):
        ...
        # ========= ① 全局上下文 =========
        # (B, d)
        H_avg = tf.reduce_mean(H, axis=[1, 2])
 
        # ========= ② 动态增量 =========
        # d_Am: (B, n, 1)
        d_Am = tf.nn.tanh(self.dyn_Am(H_avg))[:, :, None] * self.alpha
        ...
        # d_B: (B, 1, n)
        d_B = tf.nn.tanh(self.dyn_B(H_avg))[:, None, :] * self.alpha

        # ========= ③ 静态 + 动态 =========
        # curr_Am: (B, 1, n, 1)
        curr_Am = self.Am_static[None, None, :, :] + d_Am[:, None, :, :]
        curr_Am = tf.nn.softmax(curr_Am, axis=2)

        # curr_Ar: (B, 1, n, n)
        curr_Ar = self.Ar_static[None, None, :, :] + d_Ar[:, None, :, :]
        curr_Ar = tf.broadcast_to(
            curr_Ar, [B, L, self.n, self.n]
        )

        # curr_B: (B, 1, n, 1)
        curr_B = self.B_static[None, None, :, :] + d_B[:, None, :, :]
...
class MultiHeadAttention(tf.keras.layers.Layer):
  def __init__(self, d_model, num_heads):
    ...
    # Gate parameters — one per head
    self.gate_dense = tf.keras.layers.Dense(self.num_heads, activation='sigmoid')
  ...
  def call(self, q, k, v, mask):
    ...
    head_context = tf.reduce_mean(
        scaled_attention, axis=[2, 3]
    )  # [B, H]

    gate_scores = self.gate_dense(head_context)  # [B, H]
    gate_scores = tf.reshape(
        gate_scores, (batch_size, self.num_heads, 1, 1)
    )  # [B, H, 1, 1]

    gated_output = scaled_attention * gate_scores
    # broadcast over [L, D_head]
